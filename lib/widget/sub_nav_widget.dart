...
class SubNavWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Padding(padding: const EdgeInsets.all(2), child: _items(context)),
    );
  }

  Column? _items(BuildContext context) {
    if (subNavList == null) {
      return null;
    }
    List<Widget> items = [];
    ...
    //计算出第一行显示的数量
    int separate = (subNavList!.length / 2).ceil();
    return Column(
      children: [
        Row(
  ...
  Widget _item(BuildContext context, CommonModel model) {
    return Expanded(
      child: GestureDetector(
        ...
        child: Column(
          children: [
            ClipOval(child: Image.network(model.icon!, width: 18, height: 18)),
            Padding(
